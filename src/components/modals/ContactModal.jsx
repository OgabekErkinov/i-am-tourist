import { Box, Button, Stack, TextField, Typography, Snackbar, Alert, CircularProgress } from "@mui/material";
...
const ContactModal = ({ closeFunc }) => {
  ...
  const sendMessage = async (data) => {
    setLoading(true); // Loadingni boshlaymiz
  ...
  return (
    <Box id="contact" display="flex" alignItems="center" justifyContent="center" position="fixed" top={0} left={0} right={0} bottom={0} bgcolor="rgba(0,0,0,0.6)" zIndex={6}>
      <Box width={widthContainer} p={3} bgcolor="#FFFFFF" borderRadius="0.5rem" display="flex" flexDirection="column" gap={2}>
        {/* Modal sarlavha va yopish tugmasi */}
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography fontFamily="Poppins" fontWeight="700" fontSize={mdScreen ? "1rem" : "1.5rem"}>{t("sendUs")}..!</Typography>
          <Button sx={{ bgcolor: "#F97316" }} onClick={() => closeFunc(false)}>
      ...
      {alertMessage.map((alert, idx) => (
        <Snackbar key={idx} open={openAlert} autoHideDuration={3000} onClose={() => setOpenAlert(false)} anchorOrigin={{ vertical: "top", horizontal: "center" }}>
          <Alert onClose={() => setOpenAlert(false)} severity={alertSeverity} sx={{ width: "100%" }}>{alert.message}</Alert>
        </Snackbar>
      ))}
    </Box>
  );
